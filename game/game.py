...
class Game:
    def __init__(self):
        ...
        self.levels = self.define_levels() # 레벨 정의
    ...
    # <<<--- [핵심 수정] 스테이지를 대량 추가하고, 다양한 조합으로 재설계 ---
    def define_levels(self):
        center = (WIDTH // 2, HEIGHT // 2)
        # 별 모양 좌표를 미리 계산 (재사용을 위해)
        ...
        return [
            ...
            # --- 보통 (동일 모양 2개) ---
            {'templates': [{'shape':'circle', 'radius':70, 'center':(center[0]-120, center[1])}, 
                           {'shape':'circle', 'radius':70, 'center':(center[0]+120, center[1])}], 'goal':88},
            {'templates': [{'shape':'rect', 'rect':pygame.Rect(center[0]-150, center[1]-100, 100, 200)}, 
                           {'shape':'rect', 'rect':pygame.Rect(center[0]+50, center[1]-100, 100, 200)}], 'goal':88},
            
            # --- 보통 (복합 모양 2개) ---
            {'templates': [{'shape':'circle', 'radius':80, 'center':(center[0]-120, center[1])}, 
                           {'shape':'rect', 'rect':pygame.Rect(center[0]+40, center[1]-80, 160, 160)}], 'goal':85},
            {'templates': [{'shape':'star', 'points':star_points((center[0]+120, center[1]), 90)}, 
                           {'shape':'rect', 'rect':pygame.Rect(center[0]-210, center[1]-60, 120, 120)}], 'goal':82},

            # --- 어려움 (3개) ---
            {'templates': [{'shape':'circle', 'radius':60, 'center':(center[0], center[1]-110)}, 
                           {'shape':'circle', 'radius':60, 'center':(center[0]-110, center[1]+60)},
                           {'shape':'circle', 'radius':60, 'center':(center[0]+110, center[1]+60)}], 'goal':85},
            {'templates': [{'shape':'rect', 'rect':pygame.Rect(center[0]-150, center[1]-150, 100, 100)}, 
                           {'shape':'rect', 'rect':pygame.Rect(center[0]+50, center[1]-150, 100, 100)},
                           {'shape':'rect', 'rect':pygame.Rect(center[0]-50, center[1]+50, 100, 100)}], 'goal':85},
            
            # --- 전문가 (복합 3개) ---
            {'templates': [{'shape':'circle', 'radius':50, 'center':(center[0]-150, center[1])}, 
                           {'shape':'rect', 'rect':pygame.Rect(center[0]-50, center[1]-50, 100, 100)},
                           {'shape':'star', 'points':star_points((center[0]+150, center[1]), 60)}], 'goal':80},
        ]
    ...
    def start_level(self, level_index):
        ...
        initial, min_t, decrement = 60, 15, 4 # 시간 감소폭을 4초로 완만하게 조절
    ...
    def playing_loop(self):
        ...
        while self.game_state == "playing":
            ...
            remaining = max(0, self.current_level_time-elapsed)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    ...
                    is_ui_click = False
                    if done_button.is_hovered or reset_button.is_hovered or any(b.is_hovered for b in brush_buttons.values()):
                        is_ui_click = True
                    ...
                    if is_ui_click:
                        ...
                        self.stamp_dough_particle(current_mouse_pos)

                if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                    is_mouse_pressed, ui_interaction_active = False, False; self.last_mouse_pos = None

            if is_mouse_pressed and not ui_interaction_active:
                if self.last_mouse_pos and self.last_mouse_pos != current_mouse_pos:
                    self.draw_stretched_dough(self.last_mouse_pos, current_mouse_pos)
                self.stamp_dough_particle(current_mouse_pos)
                self.last_mouse_pos = current_mouse_pos
    ...
    def draw_stretched_dough(self, start_pos, end_pos):
        ...
        width = self.brush_sizes[self.brush_size]
        
        stretched_surf = pygame.transform.scale(self.dough_particle_img, (int(distance), width))
        rotated_surf = pygame.transform.rotate(stretched_surf, angle)
        ...
        self.dough_surf.blit(shadow_img, (rect.x + 3, rect.y + 3))
        self.dough_surf.blit(rotated_surf, rect)

    def stamp_dough_particle(self, pos):
        mx, my = pos
        size = self.brush_sizes[self.brush_size]
        angle = random.randint(0, 360)
        
        scaled_img = pygame.transform.scale(self.dough_particle_img, (size, size))
        img = pygame.transform.rotate(scaled_img, angle)
        rect = img.get_rect(center=(mx, my))

        shadow_img = img.copy(); shadow_img.fill(COLORS['SHADOW'], special_flags=pygame.BLEND_RGBA_MULT)
        self.dough_surf.blit(shadow_img, (rect.x + 3, rect.y + 3))
        self.dough_surf.blit(img, rect)
        
        if random.random() < 0.05:
            cs = size / 5
            cx, cy = mx + random.randint(-int(size/3), int(size/3)), my + random.randint(-int(size/3), int(size/3))
            pygame.draw.circle(self.dough_surf, COLORS['CHOCO_CHIP'], (cx, cy), cs)

    def results_loop(self):
        next_button=Button((WIDTH/2-100,HEIGHT*0.7,200,50),"다음 단계"); retry_button=Button((WIDTH/2-100,HEIGHT*0.7,200,50),"다시 도전"); menu_button=Button((WIDTH/2-100,HEIGHT*0.7+70,200,50),"메인 메뉴")
        ...
        while self.game_state == "results":
            ...
            for event in pygame.event.get():
                ...
                if success and not is_last_level:
                    if next_button.check_click(event): self.start_level(self.current_level+1)
                else:
    ...
    def end_level(self):
        if self.game_state == "playing": self.calculate_results(); self.game_state = "results"
